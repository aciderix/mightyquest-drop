...
for s in TERM KILL; do pkill -$s -f MightyQuest 2>/dev/null; pkill -$s -f winedevice 2>/dev/null; pkill -$s wineserver 2>/dev/null; pkill -$s Xvfb 2>/dev/null; pkill -$s -f http.server 2>/dev/null; done
sleep 5; rm -rf /tmp/.wine-* /tmp/.X*-lock /tmp/.X11-unix/* 2>/dev/null
# UI http server on :80 (gamedata -> GameData/Data)
cd /tmp/webroot; nohup python3 -m http.server 80 --bind 127.0.0.1 >/tmp/uihttp.log 2>&1 & disown
...
nohup wine MightyQuest.exe -server_url https://127.0.0.1 -environmentName mqel-live -branchName mqel -steamid 76561201696194782 -steamticket "" -token "" >/tmp/wine_lobby.log 2>&1 & disown
